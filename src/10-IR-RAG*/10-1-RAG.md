...
## FAQ
**1. What are the two main components of a RAG application architecture?**

A RAG application is built on two distinct phases:
- **Indexing (The Preparation Phase)**: This happens offline. Data is ingested, cleaned, and organized. It involves loading documents, splitting them into smaller chunks, and storing them in a vector database so they can be searched later.
- **Retrieval & Generation (The Runtime Phase)**: This happens when a user asks a question. The system retrieves relevant chunks from the index and passes them to the LLM to generate a final answer.

**2. What is the specific role of the "Retrieval" component?**

The Retrieval component acts as the bridge between the user's question and the database. Its only job is to find the most relevant pieces of information (context)
based on the user's query.

If the retrieval step fails and fetches irrelevant data, the LLM will likely hallucinate or provide a wrong answer. High-quality retrieval is the most important factor for accurate RAG outputs.

**3. Why must long documents be split before embedding?**

Splitting documents (chunking) is mandatory for two reasons:
- **Context Window Limits**: LLMs have a fixed memory limit. You cannot fit a 300-page manual into a single prompt, but you can fit relevant paragraphs. 
- **Search Precision**: It is much easier to find a specific answer if the database is made of small, focused concepts rather than massive, unstructured documents.

**4. What is "chunk overlap" and why is it used?**

Chunk overlap creates a "sliding window" effect where the end of one chunk is repeated at the beginning of the next. This helps ensure that the LLM has access to enough context to generate a coherent response. Without overlap, a sentence or idea might be cut in half at the split point, causing the model to lose the connection between two related statements. 

**5. What are the key components needed to build and query a vector store?**

1. **Documents**: The source material (chunked text). 
2. **Embedding Model**: The translator that turns text into vectors.
3. **Vector Database**: The warehouse (e.g., Chroma, Pinecone) that stores the vectors.
4. **Similarity Search**: The function that queries the database to find the "nearest neighbors" to your question.