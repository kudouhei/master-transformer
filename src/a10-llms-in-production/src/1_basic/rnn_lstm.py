...
# 分词和停用词过滤
try:
    tokenizer = nltk.data.tokenize.RegexpTokenizer("\w+'?\w+|\w+'")
    tokenizer.tokenize("I can't believe Sam's goin' to the party!")
    stop_words = nltk.corpus.stopwords.words("english")
except:
    nltk.download("punkt")  # NLTK 的预训练分词模型
    nltk.download("stopwords")  # NLTK 的预训练停用词模型
    spacy.cli.download("en_core_web_lg") # download spacy model english
    tokenizer = nltk.tokenize.RegexpTokenizer("\w+'?\w+|\w+'")
    tokenizer.tokenize("This is a test")
    stop_words = nltk.corpus.stopwords.words("english")     # load stop words
    nlp = spacy.load("en_core_web_lg", disable=["parser", "tagger", "ner"])

# create corpus for training 
dataset = pd.read_csv("./data/twitter.csv")

text_data = list(map(lambda x: tokenizer.tokenize(x.lower()), dataset["text"]))
...
rnn_model = RNN(INPUT_DIM, EMBEDDING_DIM, HIDDEN_DIM, OUTPUT_DIM, embedding_weights)
